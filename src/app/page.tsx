...
function Home() {
  ...
  useEffect(() => {
    fetcher("https://restcountries.com/v3.1/all").then((res) => setData(res));
  }, []);
          ...
          <div className=" flex flex-col lg:flex-row flex-wrap lg:justify-between gap-y-16">
            {data &&
              data
                ...
                .map(({ flags, name, population, region, capital }, i) => {
                  return (
...
function Search({
  ...
  const submitHandler = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (input.length > 0) {
      fetcher(`https://restcountries.com/v3.1/name/${input}`).then((data) =>
        setData(data)
      );
    } else {
      fetcher("https://restcountries.com/v3.1/all").then((res) => setData(res));
  ...
  return (
    ...
    >
      <BsSearch onClick={(e: FormEvent<HTMLFormElement>) => submitHandler(e)} />
      <input
